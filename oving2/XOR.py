...
class SigmoidModel:
    ...
    # Predetermined values - Does not Converge
    # self.W1 = torch.tensor([[0., 0.], [0., 0.]], dtype=torch.float, requires_grad=True)
    # self.b1 = torch.tensor([[0., 0.]], dtype=torch.float, requires_grad=True)
    # self.W2 = torch.tensor([[0.], [0.]], dtype=torch.float, requires_grad=True)
    # self.b2 = torch.tensor([[0.]], dtype=torch.float, requires_grad=True)

    # Predetermined values - Converges
    # self.W1 = torch.tensor([[10.0, -10.0], [10.0, -10.0]], dtype=torch.float, requires_grad=True)
    # self.b1 = torch.tensor([[-5.0, 15.0]], dtype=torch.float, requires_grad=True)
    # self.W2 = torch.tensor([[10.0], [10.0]], dtype=torch.float, requires_grad=True)
    # self.b2 = torch.tensor([[-15.0]], dtype=torch.float, requires_grad=True)
